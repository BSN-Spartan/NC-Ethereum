...
type Snapshot struct {
	...
	SignerLimitVotes []*Vote        `json:"signerLimitVotes"` // List of votes cast in chronological order
	SignerLimitTally map[uint]Tally `json:"signerLimitTally"`
}
...
func newSnapshot(config *params.CliqueConfig, sigcache *lru.ARCCache, number uint64, hash common.Hash, signers []common.Address) *Snapshot {
	snap := &Snapshot{
		...
		SignerLimit:      50,
		SignerLimitTally: make(map[uint]Tally),
	}
...
func (s *Snapshot) castSignerLimit(signerLimit uint) bool {
	...
	if old, ok := s.SignerLimitTally[signerLimit]; ok {
		...
		s.SignerLimitTally[signerLimit] = Tally{Authorize: true, Votes: 1}
	}
	return true
}
...
func (s *Snapshot) applySignerLimitVotes(signer common.Address, snap *Snapshot, header *types.Header) {
	...
	if snap.castSignerLimit(limit) {
		snap.SignerLimitVotes = append(snap.SignerLimitVotes, &Vote{
			Signer:    signer,
			Block:     number,
...
func (s *Snapshot) apply(headers []*types.Header) (*Snapshot, error) {
	...
	for i, header := range headers {
		...
		if number%s.config.Epoch == 0 {
			...
			snap.SignerLimitTally = make(map[uint]Tally)
		}
		// Delete the oldest signer from the recent list to allow it signing again
		if limit := uint64(snap.signerLimit()); number >= limit {
